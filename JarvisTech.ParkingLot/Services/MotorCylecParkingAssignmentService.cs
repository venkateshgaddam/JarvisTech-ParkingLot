...
namespace JarvisTech.ParkingLot.Services
{
    public class MotorCylecParkingAssignmentService : IParkingAssignmentService
    {
        ...
        {
            var result = await _repository.GetMotorCycleAvailableSpotsAsync(parkingLotId);
            return result;
        }
        ...
        {
            var result = await _repository.GetParkingSpotAsync(parkingLotId, VehicleType.MotorCycle.ToString());
            return result;
        }
